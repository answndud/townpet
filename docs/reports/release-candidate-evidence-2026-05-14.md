...
Production 배포 후 health는 이번 작업에서 실행하지 않았다. 실제 배포가 Ready 상태가 된 뒤 `OPS_BASE_URL=https://townpet.vercel.app corepack pnpm@9.12.3 -C app ops:check:health`로 별도 확인한다.
...
이번 로컬 증적 작업에서 실행하지 않은 항목:

- production 배포 후 public health: 실제 배포 Ready 이후 실행
...
실제 production 배포 판단은 아래 순서로 마무리한다.

1. main 배포가 Ready 상태인지 확인한다.
2. `OPS_BASE_URL=https://townpet.vercel.app corepack pnpm@9.12.3 -C app ops:check:health`를 실행한다.
3. health 실패 시 `NO-GO`로 전환하고 [배포전_on-demand_체크.md](../../business/operations/배포전_on-demand_체크.md)의 실패 대응 순서를 따른다.
