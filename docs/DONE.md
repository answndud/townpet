...
### 2026-06-07 - search term daily metric cleanup CLI wrapper 보강

- 요약: `db:cleanup:search-term-daily-metrics` wrapper가 import 시 PrismaClient/main을 실행하지 않게 하고 dry-run/apply 출력 메시지를 테스트했다.
- 변경: `runSearchTermDailyMetricCleanup(prisma)`와 `formatSearchTermDailyMetricCleanupOutput(...)`를 추가해 wrapper 출력 계약을 분리했다.
- 검증: `cleanup-search-term-daily-metrics.test.ts` PASS, file eslint PASS, `tsc --noEmit` PASS.
- 후속: search term daily metric cleanup 메시지나 run mode가 바뀌면 wrapper test를 함께 갱신한다.

### 2026-06-07 - auth audit cleanup CLI wrapper 보강

- 요약: `db:cleanup:auth-audits` wrapper가 import 시 PrismaClient/main을 실행하지 않게 하고 dry-run/apply 출력 메시지를 테스트했다.
...
### 2026-06-07 - auth/local detail visual smoke CLI wrapper 보강

- 요약: `check-auth-local-detail-visual-smoke.ts` wrapper가 fixture/browser/report/write 경계를 실DB·실브라우저 없이 검증 가능하게 됐다.
...
### 2026-06-07 - 분실/목격 작성 폼 모바일 개인정보 안내 개선

- 요약: 분실/목격 작성 폼의 “작성 전에 확인” 흐름을 보호자/목격/공유 역할 중심으로 정리했다.
- 변경: 동물 종류, 특징, 시간, 위치 필드에 짧은 도움말을 추가하고 공개 위치 기준을 위치 입력 전에 배치했다.
- 검증: targeted component test, 모바일 393px 브라우저 overflow/문구 확인, `quality:check` PASS.

### 2026-06-07 - 분실/목격 작성 완료 후 상세 공유 유도 개선
...
### 2026-06-07 - 분실/목격 랜딩 카드 역할별 CTA 개선
...
### 2026-06-08 - `/feed/guest` 서버 shell / client island 전환

- 요약: page-wide client fetch 구조를 제거하고 `/feed/guest`가 서버에서 header/control/search/list shell과 첫 목록 payload를 렌더링하게 했다.
