...
- 상태: `in_progress`
- 현재 초점: `post.service.ts`의 delete/write-support 경계 분리를 완료했고, 다음 작업은 guest-management 또는 care/market workflow 책임 분리다.
...
1. `post.service.ts`의 guest-management 또는 care/market workflow 책임 경계를 추가로 분리한다.
