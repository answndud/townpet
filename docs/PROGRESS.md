...
- Release Confidence Hardening P1-6:
  ...
  - 서른여섯 번째 slice로 ranked search document fallback candidate 조회/ranking을 `post-ranked-search-document-fallback`으로 분리했고, `post.queries`를 2573줄까지 줄였다.
  - 서른일곱 번째 slice로 list/best fetch schema compatibility fallback 실행을 `post-list-fetch-fallback`으로 분리했고, `post.queries`를 2412줄까지 줄였다.
  - 다음 작업: `listRankedSearchPosts`의 cache/run orchestration 경계 또는 post detail fallback 경계를 추가로 분리한다.
