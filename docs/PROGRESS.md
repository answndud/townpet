...
- 다음 작업은 `/` 시작페이지의 남은 추가 개선 후보 검토 후 `P0-5. SEO guide route 추가`다.
- 시작페이지 추가 개선 후보:
  - 모바일 앱 셸 상단 버튼 밀도는 줄였지만, 헤더 자체가 여전히 첫 viewport의 상당 부분을 차지한다. 향후 public landing 전용 header 여부를 검토한다.
