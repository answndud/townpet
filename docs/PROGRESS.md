...
- `/` 시작페이지에서 특정 지역명(`마포구`) 노출과 지역 허브 CTA를 제거했고, active 계획의 `마포구` 기본값도 `초기 지역 미확정` 기준으로 수정했다.

## 다음 액션

- 다음 작업은 `P0-5. SEO guide route 추가`다.
- 단, SEO guide/지역 허브 작업 전에 초기 실험 지역을 먼저 확정해야 한다. 확정 전에는 `/`과 public acquisition UI에 특정 지역명을 노출하지 않는다.
