...
- 다음 작업은 `/` 시작페이지의 추가 개선 후보 검토 후 `P0-5. SEO guide route 추가`다.
- 시작페이지 추가 개선 후보:
  - 모바일 앱 셸 상단 버튼과 피드 카테고리 pill이 홈 본문보다 먼저 크게 노출되어 첫 방문 랜딩 집중도를 떨어뜨린다.
  - 홈 피드 preview에 E2E 성격의 게시글 제목이 노출되어 production 신뢰도를 해친다. 공개 preview query 또는 seed/demo 데이터 정책을 분리해야 한다.
  - desktop에서 `지금 많이 보는 글`이 비어 있고 `최근 올라온 글`만 긴 경우 좌우 균형이 약하다. 비어 있는 컬럼은 추천 행동이나 주제별 빈 상태로 보강할 수 있다.
