...
- Release Confidence Hardening P1-6:
  ...
  - 다음 작업: ranked search SQL/helper 또는 남은 feed/list query helper를 추가 분리한다.
...
## 다음 액션
1. 다음 slice는 `post.queries`의 ranked search SQL helper 또는 남은 feed/list query helper를 별도 module로 분리한다.
