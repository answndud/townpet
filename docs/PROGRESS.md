...
- 현재 active 구현 항목 없음.
- 승인 대기 항목: production legacy upload path cleanup apply. `Post.content` 1건이며, dry-run report는 완료됐다. production mutation은 명시 승인 전에는 실행하지 않는다.
- 시작페이지 추가 개선 후보:
...
- 다음 기능 점검 후보는 legacy upload cleanup apply 승인 여부 결정이다. production apply는 별도 승인 전에는 실행하지 않는다.
