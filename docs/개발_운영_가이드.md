...
- (`build:vercel` = `production target이면 strict security-env preflight -> prisma migrate deploy(+P3005 baseline resolve) -> schema repair -> prisma generate -> next build`)
- production Vercel 배포는 필수 보안 env가 빠져 있으면 build 초반에 바로 실패합니다.
- preview에서도 같은 검증을 강제하려면 Preview env에 `DEPLOY_SECURITY_PREFLIGHT_STRICT=1`을 추가합니다.
