...
- 내부 토큰(`OPS_HEALTH_INTERNAL_TOKEN`)을 넣어 `ops:check:health`를 실행하면 `checks.search.pgTrgm` 상태를 함께 확인 가능

`pg_trgm` 필수 점검(운영 검색 품질용):

```bash
cd /Users/alex/project/townpet2/app && OPS_BASE_URL=https://townpet2.vercel.app OPS_HEALTH_INTERNAL_TOKEN=<HEALTH_INTERNAL_TOKEN> OPS_HEALTH_REQUIRE_PG_TRGM=1 pnpm ops:check:health
```

## 4-1-2. 배포 보안 env strict 프리플라이트 (복붙)

템플릿 파일:
- `/Users/alex/project/townpet2/app/.env.production.example`

배포 직전 production 보안 기준을 그대로 검증합니다.

```bash
cd /Users/alex/project/townpet2/app && pnpm ops:check:security-env:strict
```
...
```bash
cd /Users/alex/project/townpet2/app && OPS_BASE_URL=https://townpet2.vercel.app pnpm ops:perf:snapshot
```
...
```bash
cd /Users/alex/project/townpet2/app && pnpm prisma migrate deploy
```
...
알림 `이동` 클릭 시 자동 읽음 + 즉시 숨김(archive) 처리되는 흐름을 브라우저에서 검증합니다.
...
## 4-7. 알림 3일 보존 정리 작업 (복붙)

알림은 읽음/닫기 시 `archivedAt`으로 숨김 처리되고, 3일 뒤 정리 작업에서 영구 삭제됩니다.

수동 실행:

```bash
cd /Users/alex/project/townpet2/app && pnpm db:cleanup:notifications
```
...
```bash
cd /Users/alex/project/townpet2/app && pnpm ops:check:security-env
```

엄격 모드(운영 기준 강제 FAIL) 점검:

```bash
cd /Users/alex/project/townpet2/app && NODE_ENV=production SECURITY_ENV_STRICT=1 pnpm ops:check:security-env
```
...
- 기본 대상 URL:
  ...
  - 미설정 시 fallback `https://townpet2.vercel.app`
...
- `APP_BASE_URL`
- `NEXTAUTH_URL`
- `BLOB_READ_WRITE_TOKEN` (이미지 업로드 사용 시)
...
- (`build:vercel` = `(prisma migrate deploy || prisma db push) && prisma generate && next build`)

P3005(기존 DB + migration history 없음) 대응:
- 기본적으로 `migrate deploy`를 먼저 시도하고, P3005 등으로 실패하면 `db push`로 스키마를 맞춘 뒤 빌드를 계속 진행
- 서비스 안정화 후에는 `_prisma_migrations` baseline 작업을 수행하고 `migrate deploy` 단독 경로로 복귀하는 것을 권장
...
복구 순서:
1. **임시 복구(서비스 우선)**
   - Vercel `Build Command`를 잠시 아래로 설정
   - `pnpm prisma db push && pnpm prisma generate && pnpm build`
   - 배포 성공 후 기능(예: `/posts/new` 비회원 작성) 정상 동작 확인
2. **baseline 기록(운영 DB 1회)**
   - 로컬 터미널에서 운영 `DATABASE_URL`로 접속해 기존 migration들을 `applied`로 기록
   - 아래 템플릿 사용
...
3. **정상 경로 복귀**
   - Vercel `Build Command`를 아래로 되돌림
   - `pnpm prisma migrate deploy && pnpm prisma generate && pnpm build`
   - 재배포 후 `migrate deploy`가 에러 없이 끝나는지 로그 확인
...
- `BLOB_READ_WRITE_TOKEN` (Vercel Blob read/write 토큰)
