...
- 목표: 케어 요청 기능을 운영자가 반복 점검할 수 있게 문서와 로컬 데이터 준비 루틴을 만든다.
- 범위: 케어 운영 런북, 로컬 demo seed 또는 seed 지침, 수동 점검 절차, e2e 실행 위치 정리.
- 제외: 결제/보험/정산, 자동 제재, 증빙 업로드, production smoke.
- 완료 기준: 운영자가 요청/지원/완료피드백/이슈큐를 어떤 순서로 확인할지 문서화하고 로컬 재현 데이터를 준비할 수 있다.
