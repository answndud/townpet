...
- 이미 완료한 큰 경계:
  ...
  - `post.service`는 reaction/bookmark engagement service, delete service, guest-management service, market workflow service, care workflow service, write support 분리를 시작했고, 최근 기준 `post.service.ts`는 3210줄에서 1503줄까지 축소됐다.
  - Vitest config는 Vite 7 ESM 로딩 기준에 맞춰 `.mts`로 전환하고 `vite-tsconfig-paths` 의존성을 제거했다.
- 다음 액션:
  - `post.service.ts`의 create/update structured write orchestration을 추가로 분리한다.
