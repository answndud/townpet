...
- 배경:
  ...
  - `PostImage.url`, `Comment.content` 후보는 0건이다.
- 원칙:
  - production 데이터 변경은 audit 명령에서 하지 않는다.
...
- 검증:
  - cleanup dry-run script unit test
  - production dry-run report
  - apply 승인 시 `quality:check`, `ops:check:health`, affected post detail smoke
...
- 후보:
  ...
  - 상세 화면의 남은 non-primary auxiliary surface audit
