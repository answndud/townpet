...
- 목표: 매칭된 돌봄 요청을 시작/완료/취소 상태로 전환할 수 있게 한다.
- 범위: `MATCHED -> IN_PROGRESS -> COMPLETED`, 작성자 취소, 운영자 override, 상태 변경 알림, detail UI 버튼, failure-path tests.
- 제외: 세부 체크리스트, 체크인 사진, 노쇼 판정, 후기/평점, 결제/예약 보증/보험/정산, 실시간 위치 추적.
- 완료 기준: 작성자/수락 지원자/운영자 권한이 service에서 강제되고, 상태 전환 알림과 감사 로그가 남으며, 비매칭 사용자/취소 상태/잘못된 전이 테스트가 통과한다.
