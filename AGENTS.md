...
- 계획과 현재 작업 상태는 루트 `PLAN.md`에 있습니다.
...
- 앱 명령은 `app/` 기준이고, 작업 상태 문서는 루트 `PLAN.md`입니다.
- `PLAN.md`는 구조 설명이나 완료 이력이 아니라 현재와 미래의 실행 가능한 작업만 남기는 문서입니다.
...
- 새 세션은 작업 전 루트 `PLAN.md`만 먼저 읽습니다.
- 신규/진행/blocked 작업은 `PLAN.md`에 실행 가능한 slice로 기록합니다.
- 완료된 slice와 phase는 `PLAN.md`에서 제거하며 별도 완료 로그를 만들지 않습니다.
- `PLAN.md`에는 현재와 미래만 담고, 완료 이력·정책·튜토리얼·명령 출력을 남기지 않습니다.
- 첫 번째 slice가 다음 실행 작업이 되도록 위에서 아래 순서로 유지합니다.
- 활성 phase는 최대 3개, Backlog는 최대 5개로 유지합니다.
...
- 관련 문서가 동기화됨
  - 최소: `PLAN.md`
  - 필요 시: `business/security/*`, `business/policies/*`, 운영 문서
