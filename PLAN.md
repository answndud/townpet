...
|---|---|---|---|---|---|
| `contentEditable + execCommand`/실패한 Tiptap 보수 경로를 버리고 무료 외부 라이브러리 SunEditor로 게시글 작성/수정 에디터를 재구성하며, 숫자 폰트 크기/색상/이미지/링크/인용/목록을 내장 툴바 기준으로 안정화하고, 선택 텍스트만 스타일이 적용된 뒤 다음 입력은 기본 스타일로 분리되도록 마무리한다 | Codex | P0 | `in_progress` | create/edit 폼이 공용 `PostBodyRichEditor` 기반 SunEditor를 사용하고, 상단/내장 툴바에서 폰트 크기 숫자 선택과 색상 팔레트가 동작하며, `fontColor`와 `underline` 같은 인접 툴바 액션도 시각적으로 명확히 구분되고, 아이콘 내부 노드를 눌러도 글자색 submenu가 안정적으로 열리며, 열린 palette도 빈 박스가 아니라 예시처럼 8열 64색 swatch grid로 보이고 각 색 타일이 네이티브 버튼 외형이 아닌 1:1 정사각형 컬러칩으로 렌더링되며, 이미지 업로드/링크/인용/목록/게스트 수정 플로우가 새 에디터 기준 E2E로 고정되고, 저장 포맷은 기존 TownPet markup을 유지하며, 스타일 적용 뒤 다음 입력은 기본 스타일로 분리되고, `PLAN.md`/`PROGRESS.md`가 최신 상태를 반영한다 | `PLAN.md`, `PROGRESS.md`, `app/package.json`, `app/pnpm-lock.yaml`, `app/src/components/posts/post-body-rich-editor.tsx`, `app/src/components/posts/post-create-form.tsx`, `app/src/components/posts/post-detail-edit-form.tsx`, `app/src/app/globals.css`, `app/src/app/globals-css.test.ts`, `app/src/lib/post-editor-color-palette.ts`, `app/src/lib/post-editor-color-palette.test.ts`, `app/src/lib/editor-content-serializer.ts`, `app/src/lib/editor-image-markup.ts`, `app/src/lib/markdown-lite.ts`, `app/src/server/services/post.service.ts`, `app/e2e/post-editor-toolbar.spec.ts`, `app/e2e/image-upload-flow.spec.ts`, `app/e2e/guest-post-management.spec.ts`, `local PostgreSQL (Playwright verification)`, `docs/errors/2026-04-15_post-editor-font-color-submenu-target.md`, `docs/errors/2026-04-15_post-editor-color-palette-blank.md`, `docs/errors/2026-04-15_post-editor-color-palette-native-button-appearance.md`, `docs/errors/2026-04-15_post-editor-color-palette-swatch-overlay.md` |
...
| 작업명 | 담당 에이전트 | 우선순위 | 상태 | 완료기준(DoD) | 의존성 |
|---|---|---|---|---|---|
| README 개편 과정에서 남은 미사용 스크린샷과 로컬 빌드/테스트 산출물 중 프로젝트 동작에 영향 없는 파일만 정리하고, 확보 용량을 기록한다 | Codex | P3 | `done` | `README.md`와 코드에서 참조하지 않는 `docs/assets/readme/*` 잔여 자산 및 `.next`/coverage/test-results/tmp log`가 제거되고, 확보 용량이 `PROGRESS.md`에 기록된다 | `docs/assets/readme/*`, `app/.next`, `app/coverage`, `app/test-results`, `tmp-next-dev.log`, `PLAN.md`, `PROGRESS.md` |

### Cycle 403: README 문체 단정화 및 섹션명 정리 (완료)
| 작업명 | 담당 에이전트 | 우선순위 | 상태 | 완료기준(DoD) | 의존성 |
|---|---|---|---|---|---|
| 루트 README의 섹션명을 더 직접적인 표현으로 정리하고, 설명 문체를 군더더기 없는 단정형으로 다듬는다 | Codex | P3 | `done` | `README.md`에서 `왜 사업적으로 흥미로운가`, `보여주는 것`, `사용했다는 점입니다` 같은 표현이 더 직접적이고 간결한 표현으로 교체되고, 결과가 `PROGRESS.md`에 기록된다 | `README.md`, `PLAN.md`, `PROGRESS.md` |
...
| 작업명 | 담당 에이전트 | 우선순위 | 상태 | 완료기준(DoD) | 의존성 |
|---|---|---|---|---|---|
| 루트 README의 대표 화면 섹션을 작은 3칸 표에서 큰 섹션형 레이아웃으로 바꾸고, 라이브 배포 기준의 실제 화면을 `피드/검색/입양/상세/모바일`까지 다시 캡처해 반영한다 | Codex | P2 | `done` | `README.md`가 큰 이미지 섹션 구조로 바뀌고, `docs/assets/readme`에 갱신된 데스크톱/모바일 스크린샷이 추가되며, 결과가 `PROGRESS.md`에 기록된다 | `README.md`, `docs/assets/readme/*`, `PLAN.md`, `PROGRESS.md` |

### Cycle 398: README에 예비창업패키지 맥락의 프로젝트 목적 보강 (완료)
| 작업명 | 담당 에이전트 | 우선순위 | 상태 | 완료기준(DoD) | 의존성 |
|---|---|---|---|---|---|
| 루트 README에 왜 TownPet을 시작했는지에 대한 예비창업패키지 관점의 compact한 문제 정의/접근법을 추가하고, 관련 사업 문서와 톤을 맞춘다 | Codex | P2 | `done` | `README.md`에 지역 반려 정보 분산 문제, `지역 + 상황` 중심 접근, 신뢰 우선 제품 설계 이유가 2~3개 bullet 수준으로 추가되고 `PROGRESS.md`에 근거 문서와 결과가 기록된다 | `README.md`, `PLAN.md`, `PROGRESS.md`, `docs/business/지원사업_제출용_1페이지.md`, `docs/business/외부_서비스_개요.md` |
...
| 작업명 | 담당 에이전트 | 우선순위 | 상태 | 완료기준(DoD) | 의존성 |
|---|---|---|---|---|---|
| 게시글 목록 아이템 세로 여백 축소 | Codex | P2 | `done` | `feed-post-item`의 세로 패딩과 보조 메타 간격이 줄어들어 목록 한 행의 높이가 더 조밀해진다 | `PLAN.md`, `PROGRESS.md`, `app/src/components/posts/feed-infinite-list.tsx` |
