...
- [.github/workflows/quality-gate.yml](../.github/workflows/quality-gate.yml)
- [.github/workflows/ops-smoke-checks.yml](../.github/workflows/ops-smoke-checks.yml)
- [blog/19-테스트와-품질게이트-전략.md](./19-테스트와-품질게이트-전략.md)
...
> TownPet에서는 AI를 단순 자동완성 도구가 아니라, 사이클 단위 개발 시스템으로 썼습니다. 문제를 PLAN에 먼저 등록하고, 구현은 Prisma→Zod→Service→Route/UI→Tests 순서로 진행하고, 결과는 DONE과 quality-gate, ops smoke까지 연결했습니다. 그래서 기능 속도뿐 아니라 운영 추적성과 검증 가능성을 같이 가져갈 수 있었습니다.
