...
- [`app/src/lib/notification-unread-sync.ts`](../app/src/lib/notification-unread-sync.ts)
- [`app/src/server/actions/notification.ts`](../app/src/server/actions/notification.ts)
- [`app/src/server/queries/notification.queries.ts`](../app/src/server/queries/notification.queries.ts)
- [`app/src/app/notifications/redirect/[id]/route.ts`](../app/src/app/notifications/redirect/[id]/route.ts)
- [`app/src/server/queries/notification.queries.test.ts`](../app/src/server/queries/notification.queries.test.ts)
...
[`notification-bell.tsx`](../app/src/components/notifications/notification-bell.tsx)와 [`notification-center.tsx`](../app/src/components/notifications/notification-center.tsx)는 둘 다 `"use client"` 파일입니다.
...
[`app/src/server/actions/notification.ts`](../app/src/server/actions/notification.ts)의 함수들은 브라우저 컴포넌트에서 직접 호출하지만, 실제 실행은 서버에서 됩니다.
...
[`/notifications/redirect/[id]`](../app/src/app/notifications/redirect/[id]/route.ts)는 페이지가 아니라 서버 route입니다.
...
- [`notification-bell.tsx`](../app/src/components/notifications/notification-bell.tsx)
...
- [`notification-center.tsx`](../app/src/components/notifications/notification-center.tsx)
...
- [`notification-unread-sync.ts`](../app/src/lib/notification-unread-sync.ts)
...
- [`/notifications/redirect/[id]/route.ts`](../app/src/app/notifications/redirect/[id]/route.ts)
...
- [`app/scripts/e2e-notification-comment-flow.ts`](../app/scripts/e2e-notification-comment-flow.ts)
...
### Q1. 왜 전역 상태 라이브러리 대신 이벤트 기반 unread sync를 썼나요?

알림 badge 동기화 범위는 비교적 좁고, cross-tab까지 필요했습니다. 그래서 `CustomEvent + storage`가 가장 가볍고 목적에 맞았습니다.

### Q2. 알림 정합성은 어디서 보장되나요?

브라우저가 아니라 `notification.queries.ts`에서 보장합니다. 목록 조회와 unread count가 같은 규칙을 쓰도록 맞췄습니다.

### Q3. redirect route가 왜 필요한가요?

알림 대상이 이미 삭제됐거나 숨겨졌을 수 있기 때문입니다. `/notifications/redirect/[id]`가 안전한 네비게이션 게이트 역할을 합니다.
