...
- `/Users/alex/project/townpet/app/src/app/feed/guest/page.tsx`
- `/Users/alex/project/townpet/app/src/components/posts/guest-feed-page-client.tsx`
- `/Users/alex/project/townpet/app/src/server/services/posts/guest-feed-page-fetch.service.ts`
...
처음에는 guest `/feed`를 완전히 client fetch에 맡겼지만, 실제 계측을 해보니 `/api/feed/guest` 자체는 생각보다 빠르고, 첫 진입 체감은 “문서 응답 후 다시 API를 기다리는 구조”가 더 문제였습니다.

그래서 지금은:

- `/feed/guest/page.tsx`가 서버에서 초기 guest payload를 먼저 받아서
- `GuestFeedPageClient`에 `initialData`로 넘기고
- 이후 필터 변경/무한 스크롤만 client가 이어받습니다.

즉 TownPet의 guest 피드는 이제 “완전 client fetch”도 아니고 “완전 server only”도 아닙니다.
