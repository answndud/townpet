...
## 3. 게스트 피드는 canonical `/feed`로 수렴시킨다
...
하지만 지금은 첫 entry를 분리하지 않습니다.

- 첫 페이지는 canonical `/feed`가 서버에서 직접 렌더링합니다.
- `/feed/guest`는 예전 링크를 canonical `/feed`로 보내는 호환 경로입니다.
- `/api/feed/guest`는 guest 무한스크롤과 공개 read 계약을 담당합니다.

즉 TownPet는 guest와 auth의 차이를 “페이지를 두 개 만든다”로 풀지 않고, **동일한 페이지 진입 + 다른 read policy/API 계약**으로 정리했습니다.
