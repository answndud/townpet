...
## 3. 게스트 피드는 왜 아예 client page로 감쌌는가
...
게스트 피드는 `page.tsx`가 단순 `Suspense` wrapper이고, 실제 로직은 `GuestFeedPageClient`가 담당합니다.

이 구조를 쓴 이유:

1. 게스트 피드는 `searchParams` 정규화가 많습니다.
2. guest API 호출과 에러/게이트 응답을 브라우저에서 다뤄야 합니다.
3. 로그인 사용자의 `/feed`와 게스트 `/feed`를 완전히 같은 방식으로 만들 필요는 없습니다.

즉 TownPet는 “모든 페이지를 SSR로 통일”하지 않았습니다.  
초기 진입 비용과 상호작용 요구를 보고 경계를 다르게 잡았습니다.
...
- 일부 페이지는 서버에서 시작하지만 실제 핵심 로직은 client fetch에 많이 의존합니다.
