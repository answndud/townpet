...
- `block / mute`
  - 사용자 간 노출 제어입니다.
- `report`
  - 운영자 inbox로 들어가는 신고 흐름입니다.
- `sanction`
  - 실제 쓰기 권한 제한을 거는 제재 단계입니다.
- `direct moderation`
  - 운영자가 명시적으로 개입하는 경로입니다.

## 먼저 볼 핵심 파일

- [`app/src/server/services/report.service.ts`](../app/src/server/services/report.service.ts)
- [`app/src/server/services/user-relation.service.ts`](../app/src/server/services/user-relation.service.ts)
- [`app/src/server/queries/user-relation.queries.ts`](../app/src/server/queries/user-relation.queries.ts)
- [`app/src/server/services/sanction.service.ts`](../app/src/server/services/sanction.service.ts)
- [`app/src/server/services/direct-moderation.service.ts`](../app/src/server/services/direct-moderation.service.ts)
- [`app/src/server/moderation-action-log.ts`](../app/src/server/moderation-action-log.ts)
...
- [`createReport`](../app/src/server/services/report.service.ts#L243)
...
- [`assertUserInteractionAllowed`](../app/src/server/services/sanction.service.ts#L241)
...
그리고 [`app/src/server/auth.ts`](../app/src/server/auth.ts) 의 `requireCurrentUser()`도 이 함수를 호출합니다.
...
2. [`app/src/server/services/user-relation.service.ts`](../app/src/server/services/user-relation.service.ts)
3. [`app/src/server/queries/user-relation.queries.ts`](../app/src/server/queries/user-relation.queries.ts)
4. [`app/src/server/services/report.service.ts`](../app/src/server/services/report.service.ts)
5. [`app/src/server/services/sanction.service.ts`](../app/src/server/services/sanction.service.ts)
6. [`app/src/server/services/direct-moderation.service.ts`](../app/src/server/services/direct-moderation.service.ts)
7. [`app/src/app/admin/reports/page.tsx`](../app/src/app/admin/reports/page.tsx)

## 테스트와 검증

대표 검증 파일은 아래 셋입니다.

- [`app/src/server/services/report.service.test.ts`](../app/src/server/services/report.service.test.ts)
- [`app/src/server/services/direct-moderation.service.test.ts`](../app/src/server/services/direct-moderation.service.test.ts)
- [`app/src/lib/validations/direct-moderation.test.ts`](../app/src/lib/validations/direct-moderation.test.ts)

## 직접 실행해 보고 싶다면

```bash
corepack pnpm -C app test -- src/server/services/report.service.test.ts
corepack pnpm -C app test -- src/server/services/direct-moderation.service.test.ts
corepack pnpm -C app test -- src/lib/validations/direct-moderation.test.ts
```
