...
- [`app/src/server/services/guest-safety.service.ts`](../app/src/server/services/guest-safety.service.ts)
...
- [`app/src/app/api/guest/step-up/route.ts`](../app/src/app/api/guest/step-up/route.ts)
- [`app/src/app/api/posts/route.ts`](../app/src/app/api/posts/route.ts)
- [`app/src/app/api/posts/[id]/comments/route.ts`](../app/src/app/api/posts/[id]/comments/route.ts)
- [`app/src/app/api/reports/route.ts`](../app/src/app/api/reports/route.ts)
...
- [`app/src/server/guest-step-up.test.ts`](../app/src/server/guest-step-up.test.ts)
...
- [`rate-limit.ts`](../app/src/server/rate-limit.ts)
...
- [`authenticated-write-throttle.ts`](../app/src/server/authenticated-write-throttle.ts)
...
- [`POST /api/posts`](../app/src/app/api/posts/route.ts)
- [`POST /api/posts/[id]/comments`](../app/src/app/api/posts/[id]/comments/route.ts)
- [`POST /api/reports`](../app/src/app/api/reports/route.ts)
...
- [`guest-safety.service.ts`](../app/src/server/services/guest-safety.service.ts)
...
- [`guest-step-up.ts`](../app/src/server/guest-step-up.ts)
- [`/api/guest/step-up/route.ts`](../app/src/app/api/guest/step-up/route.ts)
...
- [`rate-limit.test.ts`](../app/src/server/rate-limit.test.ts)
- [`authenticated-write-throttle.test.ts`](../app/src/server/authenticated-write-throttle.test.ts)
- [`guest-step-up.test.ts`](../app/src/server/guest-step-up.test.ts)
- [`sanction.service.test.ts`](../app/src/server/services/sanction.service.test.ts)
...
### Q1. 왜 로그인 사용자와 비회원 방어선을 다르게 설계했나요?

로그인 사용자는 계정과 제재 이력을 중심으로 볼 수 있고, 비회원은 IP·fingerprint·step-up 같은 다른 신호가 더 중요합니다. 위험 모델이 다릅니다.

### Q2. step-up challenge를 왜 넣었나요?

단순 rate limit만으로는 자동화된 반복 요청을 막기 어렵기 때문입니다. 비회원 고위험 경로에는 추가 비용을 요구하는 장치가 필요했습니다.

### Q3. rate limit만으로 충분하지 않은 이유는 무엇인가요?

도배와 악성 행동은 우회 경로가 많습니다. 그래서 TownPet는 rate limit, violation 기록, 제재, step-up을 여러 층으로 겹쳤습니다.
