...
- [README.md](../README.md)
- [24-데모-문서-면접-패키지.md](./24-데모-문서-면접-패키지.md)
- [27-백엔드-포트폴리오-하드닝-로그.md](./27-백엔드-포트폴리오-하드닝-로그.md)
...
> README는 설치 가이드보다 먼저 이 프로젝트가 어떤 백엔드 역량을 보여주는지 설명하도록 구성했습니다. API route 55개 contract, production health evidence, `quality:check` 결과, post query/create 리팩터링 결과를 첫 문서에서 바로 확인할 수 있게 했습니다.
...
> 마지막으로 이 프로젝트는 기능 구현 후 끝내지 않고 검증 가능한 증거를 남겼습니다. 현재 API route 55개에 대해 method/access/validation/monitoring/test adjacency를 관리하고, `validation=none`과 `monitoring=none`은 0건입니다. `quality:check`는 lint, typecheck, Vitest 248 files / 1198 tests, Next build까지 통과했고, production health도 200 / ok로 기록했습니다.
...
- API route 55개의 contract inventory를 남겼습니다.
...
API contract는 55 route handler를 커버하고, missing method export 0건, adjacent test gap 0건, `validation=none` 0건, `monitoring=none` 0건입니다. production health는 `https://townpet.vercel.app/api/health` 기준 200 / `ok`로 기록했습니다.
...
작업을 계획, 구현, 테스트, 문서화, 운영 증거 갱신 단위로 나누고, 현재/다음 작업은 `PLAN.md`, 완료 이력은 `DONE.md`에 남깁니다. AI가 변경한 코드도 quality gate와 문서 동기화를 통과해야 완료로 봤습니다.
...
> TownPet는 반려 생활 정보를 지역과 상황 맥락으로 탐색하는 커뮤니티입니다. 저는 이 프로젝트에서 Next.js App Router를 API boundary로 쓰고, Prisma/Zod/service/query 계층을 나눠 피드, 검색, 작성, 댓글, 알림, 신고, 제재, 관리자 운영까지 구현했습니다. 특히 최근에는 API route 55개 contract, production health evidence, 1198개 Vitest가 포함된 quality gate, post query/create 리팩터링까지 정리해서 기능이 많은 프로젝트가 아니라 검증 가능한 백엔드 포트폴리오로 만드는 데 집중했습니다.
...
- “API route 55개를 contract report로 관리합니다.”
