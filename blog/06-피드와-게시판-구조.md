...
그리고 한 번 더 구조를 조정했습니다.

한동안은 guest `/feed`를 server-first로 바꿔 첫 payload를 서버에서 먼저 주입해 보기도 했습니다.
하지만 실제 배포 응답을 다시 측정해 보니, 이 구조는 `/feed` 문서 응답을 `private, no-store`로 만들고 같은 앱 안에서 `/api/feed/guest`를 한 번 더 fetch하게 만들어 **HTML 자체의 캐시 이점**을 잃게 했습니다.

그래서 최종 방향은 다시 더 단순하게 정리했습니다.

- `/feed/guest/page.tsx`는 static shell만 렌더한다
- `GuestFeedPageClient`가 브라우저에서 `/api/feed/guest`를 호출한다
- 실제 데이터 경로는 cacheable guest API가 담당한다

즉 현재 TownPet의 guest 피드는 “문서 응답은 가볍게, 데이터는 cacheable API에서”라는 경계가 더 분명합니다.

- 첫 화면: static shell + skeleton
- 데이터: cached guest API
- 이후 상호작용: client-driven

이 구조는 SSR HTML에 목록을 실어 보내지는 않지만, guest 트래픽처럼 public read가 많은 표면에서는 오히려 운영하기 쉬웠습니다.
