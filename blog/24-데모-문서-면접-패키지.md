...
- [02-townpet-전체-구조.md](./02-townpet-전체-구조.md)
- [09-검색-구조화검색-추천어.md](./09-검색-구조화검색-추천어.md)
- [14-신고-차단-제재-직접-모더레이션.md](./14-신고-차단-제재-직접-모더레이션.md)
- [19-테스트와-품질게이트-전략.md](./19-테스트와-품질게이트-전략.md)
...
> TownPet는 코드만 쌓아둔 저장소가 아니라, README로 첫인상을 만들고, 배포 사이트에서 실제 화면을 보여주고, `/admin/ops`로 운영 깊이를 증명하고, PLAN은 active 상태를, DONE은 완료 이력을 추적하게 만든 프로젝트입니다. 그래서 데모와 코드 설명, 운영 설명을 하나의 패키지처럼 보여줄 수 있습니다.
