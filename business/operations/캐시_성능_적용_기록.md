# Cache Performance Rollout (Feed/Search)

> 참고: 이 문서는 2026-03 초 실배포 측정 기록을 보존하므로, 본문에 남아 있는 `townpet2.vercel.app` 표기는 현재 서비스명과 무관한 historical reference다.

## 목적
피드/검색 응답 속도가 느린 문제를 해결하기 위해 Redis(Upstash) + CDN 캐시를 적용했고, 적용 이유/범위/데이터(측정값)를 함께 기록한다.

## 적용 배경 (데이터 기반)
운영 기준 측정값(2026-02-24, 이전 진행 로그)에서 SSR/API TTFB가 대략 0.59~0.67s 수준으로 확인됨.
- feed TTFB p50 586.7ms, p95 654.8ms
- post TTFB p50 588.8ms, p95 669.9ms
- /api/posts TTFB p50 591.0ms, p95 660.8ms
- 첫 호출 cold-ish 케이스: /api/posts TTFB 2.13s 1회 기록

출처: 2026-02-24 측정 로그. 현재 작업은 루트 `PLAN.md`, 성능 증적은 `docs/reports/`를 기준으로 관리한다.

## 왜 Redis + CDN 캐시를 선택했나
1) **반복 조회가 많음**: 비로그인 GLOBAL 피드/검색/자동완성은 동일 질의 재요청 빈도가 높다.
2) **서버리스 환경**: cold start + Prisma 초기 연결 비용이 반복 호출에서 체감된다.
3) **정합성 리스크**: 사용자별(Local/개인화/차단) 데이터는 캐시 오염 위험이 높으므로, 공용 경로부터 시작하는 것이 안전하다.

즉, **공용 읽기 경로는 캐시로 줄이고** 사용자별 경로는 단계적으로 확장하는 전략이 가장 안전하고 효과적이다.

## 적용 범위
### Redis (Query Cache)
공용/로그인/로컬까지 확장하되, **키를 사용자/동네/차단 목록까지 분리**해 정합성을 보장한다.
- 피드: `listPosts` (cursor 없음, page=1)
- 베스트: `listBestPosts`
- 검색: `listRankedSearchPosts`
- 자동완성: `listPostSearchSuggestions`
- 인기검색어: `getPopularSearchTerms`

캐시 TTL
- 피드/베스트: 30s
- 검색: 45s
- 자동완성: 60s
- 인기검색어: 300s

### CDN Cache Headers
API 응답에 `Cache-Control` 헤더를 추가해 Vercel CDN 캐시를 활용한다.
- `/api/posts`: `s-maxage=30`, `stale-while-revalidate=300`
- `/api/posts/suggestions`: `s-maxage=60`, `stale-while-revalidate=600`

## 캐시 정합성 전략
키 삭제 대신 **버전 키 증가**를 사용한다.
- 버전 키: `cache:version:{bucket}`
- 캐시 키는 `{bucket}` 버전을 포함
  - 예: `cache:feed:v{version}:{params}`

버전 증가 이벤트
- 게시글 작성/수정/삭제: feed/search/suggest
- 댓글 작성/삭제: feed
- 게시글 반응: feed
- 신고 자동 숨김/해제/일괄 처리: feed/search/suggest
- 검색어 로그 기록: popular

이 방식은 prefix delete/scan 대비 안정적이며, 즉시 무효화를 보장한다.

## 구현 변경점 (핵심 파일)
- 캐시 모듈: `app/src/server/cache/query-cache.ts`
- 피드/검색 쿼리: `app/src/server/queries/post.queries.ts`
- 인기 검색어: `app/src/server/queries/search.queries.ts`
- 캐시 무효화: `app/src/server/services/post.service.ts`, `app/src/server/services/comment.service.ts`
- CDN 헤더: `app/src/app/api/posts/route.ts`, `app/src/app/api/posts/suggestions/route.ts`
- 플래그: `app/src/lib/env.ts` (`QUERY_CACHE_ENABLED`, default true)

## 측정 계획 (배포 후 갱신 필요)
### 목표 지표
- feed TTFB p50 20%+ 개선
- /api/posts TTFB p50 20%+ 개선
- cold-ish tail 완화(2s+ 빈도 감소)

## 배포 직후 prewarm
- 1인 운영 기본 `ops-smoke-checks`는 health만 확인한다.
- `pnpm ops:prewarm`는 캐시/성능 변경, cold start 완화 검증, 성능 snapshot이 필요할 때만 on-demand로 실행한다.
- 대상 경로:
  - `/`
  - `/feed`
  - `/search`
  - `/api/posts?scope=GLOBAL`
  - `/api/feed/guest`
  - `/api/posts/suggestions?q=산책코스`
  - `/api/search/guest?q=강아지`
  - `/api/lounges/breeds/golden/posts?q=산책`
- 목적:
  - 첫 사용자 요청 전에 공개 GET 경로를 2회 호출해 cold start/cache MISS를 완화
  - 특히 공개 API 경로의 2번째 요청을 `HIT` 상태로 빠르게 전환
- 2026-03-06 실배포(`https://townpet2.vercel.app`) 수동 검증:
  - `feed_page_guest`: 1차 `2955ms MISS` -> 2차 `297ms MISS`
  - `api_posts_global`: 1차 `1357ms MISS` -> 2차 `123ms HIT`
  - `api_posts_suggestions`: 1차 `273ms MISS` -> 2차 `128ms HIT`
  - `api_breed_posts`: 1차 `351ms MISS` -> 2차 `121ms HIT`
- 해석:
  - 공개 API는 prewarm 효과가 분명함
  - `/feed`, `/search`는 2차에도 `MISS`가 남을 수 있어, 현재는 "페이지 CDN HIT 보장"이 아니라 "cold path 완화" 용도로 본다

## 현재 확인된 구조적 제약 (2026-03-06)
- 실배포 헤더 재확인 결과:
  - `/feed`
  - `/search`
  - `/feed/guest`
  - `/search/guest`
  - `/posts/:id/guest`
  - 모두 `cache-control: private, no-cache, no-store, max-age=0, must-revalidate`
- 핵심 원인:
  - `middleware.ts`가 요청마다 새 CSP nonce(`x-nonce`, `x-csp-nonce`)를 발급한다.
  - Next HTML 응답의 script/preload nonce도 요청마다 달라진다.
  - 따라서 guest rewrite와 `Cache-Control` 헤더만으로는 HTML 응답을 CDN public cache로 만들 수 없다.
- 의미:
  - 현재 성능 개선의 실효 영역은 HTML page cache가 아니라 API cache + prewarm + 서버 동적 의존 축소다.
  - HTML 공개 캐시를 계속 밀려면 CSP nonce 전략 자체를 다시 설계해야 한다.
  - 보안 우선순위를 유지하려면, 다음 단계는 CSP를 약화시키기보다 guest 화면을 정적 shell + 클라이언트 데이터 로딩으로 옮기는 방식이 더 안전하다.

## 랜딩 페이지 재생성 outlier 완화 (2026-06-06)
- production `/` 반복 측정에서 warm p50은 `~108ms`였지만 첫 STALE/PRERENDER성 응답이 `800ms대`로 튀는 사례가 반복됐다.
- `ops:prewarm` 대상에 `/`를 추가해 배포 직후 landing page도 예열한다.
- `/`의 `revalidate`는 `60s`에서 `300s`로 늘린다.
- 이유:
  - 홈의 live board는 사용자 획득용 preview라 초 단위 최신성이 필요하지 않다.
  - 5분 stale은 제품상 허용 가능하고, 재생성 빈도를 낮춰 field LCP/FCP outlier 노출 가능성을 줄인다.
  - 실제 피드 최신성은 `/feed/guest`와 관련 API에서 계속 확인할 수 있다.

## guest 공개 API 배포 검증 (2026-03-06)
- 대상:
  - `/api/feed/guest`
  - `/api/search/guest?q=강아지`
- 확인 결과:
  - 첫 요청은 `x-vercel-cache: MISS`
  - 직후 재요청에서는 두 경로 모두 `x-vercel-cache: STALE`, `age` 증가 확인
  - 응답 헤더는 `cache-control: public`만 노출되지만, Vercel CDN 재사용은 실제로 동작함
- 해석:
  - guest `/feed`, `/search` HTML은 여전히 비공개 캐시지만, 초기 데이터는 공개 API cache로 분리된 상태
  - 따라서 현재 구조에서 체감 개선의 핵심은 HTML cache가 아니라 guest 초기 데이터 API cache와 prewarm이다

## guest API 자동화 확장 (2026-03-06)
- prewarm 대상 추가:
  - `/api/feed/guest`
  - `/api/search/guest?q=강아지`
- 실배포 prewarm 결과:
  - `api_feed_guest`: 1차 `967ms MISS` -> 2차 `118ms HIT`
  - `api_search_guest`: 1차 `555ms MISS` -> 2차 `111ms HIT`
  - 같은 실행에서 `/feed`, `/search`도 2차 `HIT` 확인
- 의미:
  - guest HTML 자체가 캐시 제약을 받더라도, 초기에 필요한 공개 데이터는 배포 직후 빠르게 `HIT` 상태로 전환 가능
  - 운영 자동화가 guest API까지 포함되면서 cold start 완화 범위가 넓어짐

## 최신 latency snapshot (2026-03-06)
- 실행:
  - `OPS_BASE_URL=https://townpet2.vercel.app pnpm -C app ops:perf:snapshot`
- steady-state p95(ms)
  - `api_feed_guest`: `228.4`
  - `api_search_guest`: `215.2`
  - `api_breed_posts`: `194.6`
  - `api_posts_global`: `374.0`
  - `api_posts_suggestions`: `226.5`
  - `api_search_log`: `299.3`
- warm-up tail
  - `api_posts_global`: 첫 샘플 TTFB `3083.0ms`
  - `api_search_log`: 첫 샘플 TTFB `566.5ms`
- 결론:
  - steady-state는 계속 threshold PASS
  - 남은 주의점은 cold/warm-up 첫 요청 tail latency

### 측정 방법
1) curl 15회 반복
- `time_starttransfer` / `time_total` 수집
- 대상: `/feed`, `/api/posts?scope=GLOBAL`, `/search?q=...`

2) Playwright 라운드트립
- feed -> post -> back 15회 반복

결과는 이 문서의 "측정 결과" 섹션에 업데이트한다.

## 측정 결과 (추가 예정)
### 2026-02-26 (curl 15회, 배포 전 baseline)
- 대상 URL
  - https://townpet2.vercel.app/feed
  - https://townpet2.vercel.app/api/posts?scope=GLOBAL
  - https://townpet2.vercel.app/search?q=%EC%82%B0%EC%B1%85%EC%BD%94%EC%8A%A4
- 결과(ms)
  - feed: TTFB p50 569.1, p95 1379.9 / total p50 683.9, p95 1767.9
  - api_posts: TTFB p50 514.0, p95 1035.2 / total p50 514.5, p95 1037.9
  - search: TTFB p50 502.1, p95 661.9 / total p50 534.1, p95 684.7

### 2026-02-26 (curl 15회, 캐시 배포 후)
- 대상 URL
  - https://townpet2.vercel.app/feed
  - https://townpet2.vercel.app/api/posts?scope=GLOBAL
  - https://townpet2.vercel.app/search?q=%EC%82%B0%EC%B1%85%EC%BD%94%EC%8A%A4
- 결과(ms)
  - feed: TTFB p50 523.3, p95 578.1 / total p50 675.3, p95 724.7
  - api_posts: TTFB p50 535.4, p95 672.1 / total p50 536.1, p95 674.2
  - search: TTFB p50 484.3, p95 710.0 / total p50 499.5, p95 716.5

### 2026-02-26 (curl 15회, 배포 통과 후 재측정)
- 대상 URL
  - https://townpet2.vercel.app/feed
  - https://townpet2.vercel.app/api/posts?scope=GLOBAL
  - https://townpet2.vercel.app/search?q=%EC%82%B0%EC%B1%85%EC%BD%94%EC%8A%A4
- 결과(ms)
  - feed: TTFB p50 549.0, p95 937.8 / total p50 680.9, p95 1119.0
  - api_posts: TTFB p50 284.6, p95 594.8 / total p50 290.9, p95 606.7
  - search: TTFB p50 511.4, p95 739.0 / total p50 525.4, p95 746.1

### 2026-02-26 (curl 30회, 꼬리 지연 완화 배포 후)
- 대상 URL
  - https://townpet2.vercel.app/feed
  - https://townpet2.vercel.app/api/posts?scope=GLOBAL
  - https://townpet2.vercel.app/search?q=%EC%82%B0%EC%B1%85%EC%BD%94%EC%8A%A4
- 결과(ms)
  - feed: TTFB p50 525.5, p95 749.0 / total p50 671.4, p95 910.4
  - api_posts: TTFB p50 284.8, p95 363.5 / total p50 287.3, p95 370.7
  - search: TTFB p50 496.5, p95 619.2 / total p50 503.2, p95 626.5

### 2026-02-26 (curl 30회, read rate-limit 캐시/guest SSR 캐시 배포 후)
- 대상 URL
  - https://townpet2.vercel.app/feed
  - https://townpet2.vercel.app/api/posts?scope=GLOBAL
  - https://townpet2.vercel.app/search?q=%EC%82%B0%EC%B1%85%EC%BD%94%EC%8A%A4
- 결과(ms)
  - feed: TTFB p50 451.1, p95 707.6 / total p50 630.3, p95 878.5
  - api_posts: TTFB p50 237.9, p95 360.7 / total p50 242.6, p95 367.0
  - search: TTFB p50 447.4, p95 581.6 / total p50 455.8, p95 589.4

### 2026-02-26 (curl 30회, guest /feed CDN 캐시 배포 후)
- 대상 URL
  - https://townpet2.vercel.app/feed
  - https://townpet2.vercel.app/api/posts?scope=GLOBAL
  - https://townpet2.vercel.app/search?q=%EC%82%B0%EC%B1%85%EC%BD%94%EC%8A%A4
- 결과(ms)
  - feed: TTFB p50 472.1, p95 584.7 / total p50 610.2, p95 690.3
  - api_posts: TTFB p50 238.6, p95 289.3 / total p50 243.1, p95 295.6
  - search: TTFB p50 459.9, p95 594.0 / total p50 463.4, p95 594.5

### 2026-02-26 (Playwright feed scroll perf)
- 보고서: `business/reports/피드_스크롤_성능_리포트.md`
- 판정: PASS
- 요약: p95 프레임 17.5ms, Jank 1.12%, 100ms+ 프레임 0회

### 전회 대비 변화(2026-02-24 baseline)
- feed TTFB p50: 586.7ms -> 523.3ms (약 -10.8%)
- api_posts TTFB p50: 591.0ms -> 535.4ms (약 -9.4%)
- search TTFB p50: (baseline 없음) -> 484.3ms

### 배포 전 대비 변화(2026-02-26 baseline)
- feed TTFB p50: 569.1ms -> 523.3ms (약 -8.1%)
- api_posts TTFB p50: 514.0ms -> 535.4ms (약 +4.2%)
- search TTFB p50: 502.1ms -> 484.3ms (약 -3.5%)

## 운영 주의사항
- 캐시는 사용자/동네/차단 목록을 키에 포함해 Local/로그인 경로도 안전하게 분리한다.
- Upstash 미설정 시 메모리 캐시로 fallback한다. (멀티 인스턴스에서는 효율이 떨어질 수 있음)
- 필요 시 `QUERY_CACHE_ENABLED=0`으로 즉시 비활성화 가능.

## 추가 개선(2026-02-26)
- feed/search SSR에서 독립 요청을 병렬화해 서버 대기 시간을 단축.
- 커뮤니티 목록/게스트 읽기 정책 조회에 단기 캐시(TTL 60~120s) 적용.
- 검색 후보군 상한을 축소해 p95 꼬리 지연 완화 시도.
- 자동완성 요청에서 클라이언트 `no-store` 제거(Edge 캐시 활용).
- guest 컨텍스트는 `unstable_cache`로 재사용(비로그인 SSR 비용 절감).
- read rate-limit은 1초 이내 중복 호출을 메모리에서 스킵(Upstash RTT 감소).
- 비로그인 `/feed`는 CDN 캐시 헤더 적용(LOCAL/개인화 제외).
- 게시글 상세/댓글은 guest 경로에 Redis 캐시(30s) 적용.

## 추가 개선(2026-03-06)
- guest `/feed`의 첫 페이지는 `/api/feed/guest`를 사용했지만, 무한스크롤 추가 로드는 기본 `/api/posts`를 타고 있었다.
- `FeedInfiniteList`의 guest feed 경로를 `/api/feed/guest`로 일원화했다.
- `/api/feed/guest`에 `cursor` 전용 compact 응답(`items`, `nextCursor`)을 추가해 다음 페이지 로드 시:
  - 커뮤니티 네비 목록 조회 생략
  - 베스트/페이지 메타 계산 생략
  - guest 전용 캐시 헤더 유지
- 의미
  - `api_posts_global` cold tail이 남아 있어도, guest `/feed`의 실제 무한스크롤 hot path는 더 직접적으로 영향을 덜 받는다.
