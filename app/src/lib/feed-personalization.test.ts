...
describe("feed personalization helpers", () => {
  ...
  it("surfaces preferred communities as secondary signal in feed summary", () => {
    ...
    expect(buildFeedPersonalizationSummary(context)).toMatchObject({
      ...
      emphasis:
          "세그먼트 신뢰도 83% · 게시판 강아지 일상, 강아지 건강 · 관심 주제 산책, 건강 · 최근 반응 산책, 후기 · 최근 클릭/광고 반응 말티즈, 산책 · 최근 오래 읽은 글 건강, 산책 · 최근 북마크 사료, 간식",
    });
  ...
  it("falls back to preferred communities when profile signals are missing", () => {
    ...
    expect(buildFeedPersonalizationSummary(context)).toMatchObject({
        title: "게시판 기준으로 기본 맞춤 추천 중",
      emphasis:
          "게시판 강아지 일상, 강아지 건강 · 관심 주제 산책, 건강 · 최근 반응 산책, 후기 · 최근 클릭/광고 반응 말티즈, 산책 · 최근 오래 읽은 글 건강, 산책 · 최근 북마크 사료, 간식",
    });
  });
  ...
  it("falls back to interest-tag summary when only content interests are available", () => {
    ...
    expect(buildFeedPersonalizationSummary(context)).toMatchObject({
      ...
      emphasis:
          "관심 주제 산책, 건강 · 최근 반응 산책, 후기 · 최근 클릭/광고 반응 말티즈, 산책 · 최근 오래 읽은 글 건강, 산책 · 최근 북마크 사료, 간식",
    });
  });
  ...
  it("falls back to recent behavior summary when only click/ad activity is available", () => {
    const context = resolveFeedAudienceContext({
      recentBehaviorLabels: ["말티즈", "산책"],
    });
