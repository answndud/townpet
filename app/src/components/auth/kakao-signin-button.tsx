...
export function KakaoSignInButton({
  ...
  const handleClick = () => {
    ...
    startTransition(async () => {
      await signIn("kakao", { callbackUrl: safeCallbackUrl });
    });
  };
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={isPending}
      className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-md border border-[#d7bd24] bg-[#fee500] px-4 py-2 text-sm font-semibold text-[#3c1e1e] transition hover:bg-[#f5dd00] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2f66ba]/40 disabled:cursor-not-allowed disabled:opacity-70 sm:min-h-11"
    >
