...
import { getAnimalBoardByCode, ANIMAL_BOARD_CATALOG, buildAnimalBoardHref, type AnimalBoardType } from "@/lib/animal-board-catalog";
...
export async function AnimalBoardPage({ code, boardType, searchParams }: { code: string; boardType?: AnimalBoardType; searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="tp-page-bg min-h-screen"><div className="mx-auto w-full max-w-[1320px] px-4 py-5 sm:px-6 lg:px-10">
    ...
    <nav className="mt-3 flex gap-1.5 overflow-x-auto border-b border-[#dbe6f6] pb-2" aria-label="동물 게시판 유형">{tabs.map((tab) => <Link key={tab.slug || "all"} href={`${hrefForTab(tab.slug)}${searchSuffix}`} className="tp-filter-pill whitespace-nowrap px-3 py-2 text-xs">{tab.label}</Link>)}</nav>
    ...
    <div className="mt-4 flex flex-wrap gap-2">{ANIMAL_BOARD_CATALOG.filter((candidate) => candidate.code !== code).slice(0, 6).map((candidate) => <Link key={candidate.code} href={buildAnimalBoardHref(candidate.code)} className="tp-btn-soft px-3 py-2 text-xs">{candidate.label} 게시판</Link>)}</div>
  </div></main>;
}
