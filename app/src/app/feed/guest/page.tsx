...
export default async function Home({ searchParams }: HomePageProps) {
  ...
  const { loginRequiredTypes } = await getGuestFeedContext();
  const isAuthenticated = false;
  const blockedTypesForGuest = loginRequiredTypes;
  ...
  const requestedPetTypeIds =
    normalizedPetTypeValues.length > 0
      ...
      : requestedPetTypeId
        ...
        : [];
  const defaultPetTypeIds = allPetTypeIds;
  ...
  if (isLocalRequiredType && type) {
    return (
      <NeighborhoodGateNotice
        ...
        description={`${postTypeMeta[type].label} 게시판은 내 동네 기반으로 노출됩니다. 로그인 후 대표 동네를 설정해 주세요.`}
        primaryLink={loginHref(`/feed?type=${type}`)}
        primaryLabel="로그인하기"
        secondaryLink="/feed"
  ...
  const totalItemCount =
    mode === "BEST" && !isGuestTypeBlocked
      ? await countBestPosts({
          ...
          excludeTypes: blockedTypesForGuest,
          neighborhoodId,
          minLikes: 1,
          viewerId: undefined,
        }).catch((error) => {
