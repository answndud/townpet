...
export default async function SavedPostsPage({ searchParams }: SavedPostsPageProps) {
  ...
  const makeHref = ({
    ...
    return serialized ? `/saved?${serialized}` : "/saved";
  };
    ...
    <div className="tp-page-bg min-h-screen pb-16">
      <main className="mx-auto flex w-full max-w-[1320px] flex-col gap-5 px-4 py-6 sm:px-6 lg:px-10">
        <header className="tp-hero p-5 sm:p-6">
          <p className="text-[11px] uppercase tracking-[0.24em] text-[#3f5f90]">저장한 글</p>
          <h1 className="mt-2 text-2xl font-bold tracking-tight text-[#10284a] sm:text-3xl">
            나중에 다시 볼 게시글
          </h1>
          <p className="mt-2 text-sm text-[#4f678d]">
            피드나 상세에서 저장한 글을 한곳에서 모아 확인할 수 있습니다.
          </p>
        </header>

        <section className="tp-card p-4 sm:p-5">
          <div className="space-y-3">
            <form action="/saved" className="flex flex-col gap-2 sm:flex-row sm:items-center">
        ...
        <section className="tp-card overflow-hidden">
          {posts.length === 0 ? (
            <EmptyState
              title="저장한 글이 없습니다"
              description="피드나 상세에서 저장 버튼을 눌러 나중에 다시 볼 글을 모아보세요."
              actionHref="/feed"
              actionLabel="피드로 이동"
            />
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-xs text-[#4f678d] md:text-right">
                      ...
                      <p className="mt-1">저장 {formatRelativeDate(post.bookmarkedAt)}</p>
