  ...
  return (
    <div className="tp-page-bg min-h-screen pb-16">
      <main className="mx-auto flex w-full max-w-[1320px] flex-col gap-5 px-4 py-6 sm:px-6 lg:px-10">
        <section className="tp-hero p-5 sm:p-6" aria-label="내 작성글 화면 로딩 중">
          <Skeleton className="h-3 w-24 rounded-full" />
          <Skeleton className="mt-4 h-8 w-60 rounded-lg" />
          <Skeleton className="mt-3 h-4 w-full max-w-[380px] rounded-full" />
        </section>
          ...
          <div className="tp-soft-card mt-3 p-3">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {Array.from({ length: 8 }).map((_, index) => (
                <Skeleton
                  key={`my-posts-filter-loading-${index}`}
        ...
        <section className="tp-card overflow-hidden" aria-hidden="true">
          <div className="divide-y divide-[#e1e9f5]">
            {Array.from({ length: 3 }).map((_, index) => (
              <div
                key={`my-posts-row-loading-${index}`}
