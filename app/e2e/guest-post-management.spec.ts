...
import { DEFAULT_GUEST_POST_POLICY } from "../src/lib/guest-post-policy";
...
test.describe("guest post management", () => {
  ...
  test("guest creates, edits, and deletes post with password", async ({ page }) => {
    const originalPolicy = await getGuestPostPolicy();
    ...
    try {
      ...
      await prisma.guestViolation.deleteMany();
      await setGuestPostPolicy(DEFAULT_GUEST_POST_POLICY);
