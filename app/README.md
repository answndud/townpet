...
- 1인 운영 production smoke는 `OPS_BASE_URL=https://townpet.vercel.app pnpm ops:evidence:solo`로 health만 확인하고, 결과는 `../docs/reports/ops-evidence-*.md`에 로컬로 남깁니다.
- 배포 전에는 `../business/operations/배포전_on-demand_체크.md`에 따라 `quality:check -> 변경 범위별 e2e -> 배포 -> ops health` 순서로 확인합니다.
