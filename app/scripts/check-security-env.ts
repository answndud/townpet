...
export type SecurityEnvProfile = "full" | "build";
...
type EvaluateSecurityEnvOptions = {
  env?: NodeJS.ProcessEnv;
  profile?: SecurityEnvProfile;
  fetchImpl?: typeof fetch;
};

type SecurityEnvEvaluation = {
  failed: CheckResult[];
  nodeEnv: string;
  profile: SecurityEnvProfile;
  results: CheckResult[];
...
function resolveAuthSecret(env: NodeJS.ProcessEnv) {
  return read(env, "AUTH_SECRET") || read(env, "NEXTAUTH_SECRET");
}

export function resolveSecurityEnvProfile(
  args: string[] = process.argv.slice(2),
  env: NodeJS.ProcessEnv = process.env,
): SecurityEnvProfile {
  const cliProfile = args.find((arg) => arg.startsWith("--profile="))?.slice("--profile=".length);
  const rawProfile = (cliProfile ?? env.SECURITY_ENV_PROFILE ?? "full").trim().toLowerCase();
  return rawProfile === "build" ? "build" : "full";
}

function shouldRunRemoteHealthCheck(profile: SecurityEnvProfile) {
  return profile === "full";
}

export async function evaluateSecurityEnv(
  options: EvaluateSecurityEnvOptions = {},
): Promise<SecurityEnvEvaluation> {
  ...
  const demoUserEmail = read(env, "DEMO_USER_EMAIL");
  results.push({
  ...
  const socialDevLoginEnabled = hasTruthyFlag(read(env, "ENABLE_SOCIAL_DEV_LOGIN"));
  results.push({
  ...
  const guestHashPepper = read(env, "GUEST_HASH_PEPPER");
  results.push({
    key: "GUEST_HASH_PEPPER",
    status: guestHashPepper ? "PASS" : enforceProdRules ? "FAIL" : "WARN",
    detail: guestHashPepper ? "설정됨" : "미설정 시 legacy SHA-256 fallback 동작",
  });

  const healthInternalToken = read(env, "HEALTH_INTERNAL_TOKEN");
  results.push({
  ...
  results.push({
    ...
    detail: resendApiKey ? "설정됨" : "미설정 시 인증/비밀번호 재설정 메일이 전송되지 않음",
  });
  ...
  results.push({
    ...
    detail: blobToken ? "설정됨" : "미설정 시 hosted runtime 이미지 업로드가 500으로 실패",
  });

  if (shouldRunRemoteHealthCheck(profile)) {
    const opsBaseUrl = read(env, "OPS_BASE_URL");
    const opsHealthToken = read(env, "OPS_HEALTH_INTERNAL_TOKEN");
    if (!opsBaseUrl) {
      results.push({
      ...
      try {
        ...
        const response = await fetchImpl(`${opsBaseUrl.replace(/\/$/, "")}/api/health`, {
          method: "GET",
          headers,
          cache: "no-store",
        });
...
export async function main() {
  const profile = resolveSecurityEnvProfile();
  const evaluation = await evaluateSecurityEnv({ profile });
  const { failed, nodeEnv, results, strictMode, warned } = evaluation;
